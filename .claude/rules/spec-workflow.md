...
```
THE System SHALL {動作}
WHEN {条件}の場合、THE System SHALL {動作}
IF {条件} THEN THE System SHALL {動作}
```
...
2. WHEN ユーザーが未認証の場合、THE System SHALL サインインボタンを表示する
...
**criteria.mdの場所:**
`.kiro/specs/{feature_name}/criteria.md`
...
export function createEntry(input: CreateEntryInput): Promise<Entry> {
  ...
  return db.transaction.create({ data: { ...input, userId } });
}
```

✅ **良い例（言葉で説明）:**
- 関数名: `createEntry`
- 入力: `CreateEntryInput`型（金額、日付、説明、カテゴリID）
- 出力: `Promise<Entry>`
- 処理フロー:
  1. `getCurrentUserId()`を呼び出し、現在のユーザーIDを取得
  2. 入力データに`userId`を追加
  3. `db.transaction.create()`でデータベースに記録を作成
  4. 作成した記録を返す
- エラーハンドリング: 認証エラーの場合、UNAUTHORIZEDエラーをスロー

### 4. ユーザー承認の必須化

各フェーズの完了時に**必ずユーザーの承認を得る:**

```typescript
// 例: requirements.mdの承認を得る
userInput({
  question: "**要件定義の内容は問題ありませんか？修正が必要な箇所があれば教えてください。問題なければ、次は基本設計に進みます。**",
...
**標準的なcriteria.mdの参照先:**
`.kiro/specs/weekly-kakeibo-app/criteria.md`
...
### プロジェクト内ドキュメント
- [.kiro/specs/weekly-kakeibo-app/criteria.md](../../.kiro/specs/weekly-kakeibo-app/criteria.md) - 設計書の品質基準
- [.kiro/specs/weekly-kakeibo-app/requirements.md](../../.kiro/specs/weekly-kakeibo-app/requirements.md) - 要件定義の例
- [.kiro/specs/weekly-kakeibo-app/design.md](../../.kiro/specs/weekly-kakeibo-app/design.md) - 設計サマリーの例
- [.kiro/steering/spec-workflow-rules.md](../../.kiro/steering/spec-workflow-rules.md) - Specワークフロールール詳細
